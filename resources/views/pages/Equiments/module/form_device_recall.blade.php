<div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvas_device_recall" aria-labelledby="offcanvasRightLabel"
    style="z-index: 99999">
    <div class="offcanvas-header text-center">
        <button type="button" style="margin-left:2% " class="btn-close text-reset" data-bs-dismiss="offcanvas"
            aria-label="Close"><i class="fa fa-times" aria-hidden="true"></i></button>
        <h3 style="margin-right: 45%" id="offcanvasRightLabel">Cấp Phát Thiết Bị</h3>
    </div>
    <div class="offcanvas-body">
        <form id="allocation_form_submit" class="row d-flex mt-5 justify-content-center" method="post"
            enctype="multipart/form-data">
            <div class="col-md-12 mb-4 row  d-flex justify-content-center">
                <div class="col-3 row d-flex justify-content-center text-center imgcover">
                    <img class="col-12" width="210" height="200" id="img_user_device_recall"
                        src="./img/avatar2.png" alt="avatar">
                    <span class="text-sm border row mt-1" style="width: 59%;border-radius:10px;">
                        <div class="col-2 d-flex justify-content-center align-items-center">
                            <img width="10" height="10"
                                src="data:image/png;base64,"
                                alt="">
                        </div>
                        Đang Hoạt Động
                    </span>
                </div>
                <div class="card col-4 justify-content-center">
                    <div class="form-group">
                        <label for="device_recall_user_code" class="col-4 col-form-label w-100">Mã Nhân Sự</label>
                        <input class="form-control border-0 " name="device_recall_user_code"
                            id="device_recall_user_code" disabled="">
                    </div>
                    <div class="form-group">
                        <label for="device_recall_user_name" class="col-4 col-form-label w-100">Nhân Sự</label>
                        <input class="d-none " name="personnel_id_allocation" id="personnel_id_allocation">
                        <input class="form-control " name="device_recall_user_name" id="device_recall_user_name"
                            placeholder="Tên nhân sự..." disabled="">
                    </div>
                </div>

                <div class="col-3 row d-flex justify-content-center" style=" margin-top: 16px; margin-bottom: 0px;">
                    <div class="form-group">
                        <label for="device_recall_user_phongban" class="col-4 col-form-label w-100">Phòng Ban</label>
                        <input class="form-control  border-0" name="device_recall_user_phongban"
                            id="device_recall_user_phongban" disabled="">
                    </div>
                    <div class="form-group">
                        <label for="device_recall_user_chucvu" class="col-4 col-form-label w-100">Chức Vụ</label>
                        <input class="form-control border-0" name="device_recall_user_chucvu"
                            id="device_recall_user_chucvu" disabled="">
                    </div>
                </div>
            </div>
            <div class="col-md-12 row mt-4">
                <div class="table-responsive col-12">
                    <table class="table table-hover	">
                        <colgroup>
                            <col width="150" span="1">
                            <col width="150" span="1">
                        </colgroup>
                        <caption></caption>
                        <thead class="table-light">
                            <tr>
                                <th class="align-items-center">Mã Thiết Bị</th>
                                <th class="align-items-center">Mã Sản Phẩm</th>
                                <th class="text-center">Tên Sản Phẩm</th>
                                <th class="text-center">Trạng Thái</th>
                                <th class="text-center">Chọn</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td class="text-center">SKU0001</td>
                                <td class="text-center">LAP00001</td>
                                <td class="text-center">Laptop Dell</td>
                                <td class="text-center"><span class="badge rounded-pill bg-success">Success</span< /td>
                                <td class="text-center"><input type="checkbox" name="" id=""></td>
                            </tr>
                            <tr>
                                <td class="text-center">SKU0001</td>
                                <td class="text-center">LAP00001</td>
                                <td class="text-center">Laptop Dell</td>
                                <td class="text-center"><span class="badge rounded-pill bg-success">Success</span< /td>
                                <td class="text-center"><input type="checkbox" name="" id=""></td>
                            </tr>
                            <tr>
                                <td class="text-center">SKU0001</td>
                                <td class="text-center">LAP00001</td>
                                <td class="text-center">Laptop Dell</td>
                                <td class="text-center"><span class="badge rounded-pill bg-success">Success</span<
                                            /td>
                                <td class="text-center"><input type="checkbox" name="" id=""></td>
                            </tr>
                            <tr>
                                <td class="text-center">SKU0001</td>
                                <td class="text-center">LAP00001</td>
                                <td class="text-center">Laptop Dell</td>
                                <td class="text-center"><span class="badge rounded-pill bg-success">Success</span<
                                            /td>
                                <td class="text-center"><input type="checkbox" name="" id=""></td>
                            </tr>
                            <tr>
                                <td class="text-center">SKU0001</td>
                                <td class="text-center">LAP00001</td>
                                <td class="text-center">Laptop Dell</td>
                                <td class="text-center"><span class="badge rounded-pill bg-success">Success</span<
                                            /td>
                                <td class="text-center"><input type="checkbox" name="" id=""></td>
                            </tr>
                            <tr>
                                <td class="text-center">SKU0001</td>
                                <td class="text-center">LAP00001</td>
                                <td class="text-center">Laptop Dell</td>
                                <td class="text-center"><span class="badge rounded-pill bg-success">Success</span<
                                            /td>
                                <td class="text-center"><input type="checkbox" name="" id=""></td>
                            </tr>
                            <tr>
                                <td class="text-center">SKU0001</td>
                                <td class="text-center">LAP00001</td>
                                <td class="text-center">Laptop Dell</td>
                                <td class="text-center"><span class="badge rounded-pill bg-success">Success</span<
                                            /td>
                                <td class="text-center"><input type="checkbox" name="" id=""></td>
                            </tr>
                        </tbody>
                        <tfoot>
                            <h4 class="text-center">Danh Sách Thiết Bị</h4>
                        </tfoot>
                    </table>
                </div>
            </div>
            <hr>
            <div class="wrapper col-md-12 mt-5 d-flex justify-content-around">
                <button type="submit" class="btn btn-success">Thu Hồi</button>
            </div>
        </form>
    </div>
</div>
